...
/**
 * Convert OME-XML GenericExcitationSource object to list of TagData. 
 * @author Susanne Kunis<susannekunis at gmail dot com>
 *
 */
public class GenericExcitationSourceConverter extends DataConverter{
	...
	public LinkedHashMap<String, TagData> convertData(GenericExcitationSource l,LightSourceSettings settings)
	{
		...
		if(l!=null) {
			...
			try{ tagMap.put(TagNames.MAP,convertMap(l.getMap(),REQUIRED));
			} catch (NullPointerException e) {
				tagMap.put(TagNames.MAP,convertMap(null,REQUIRED));
			}	
		}else {
			tagMap.put(TagNames.ID,convertID(null,REQUIRED));
			tagMap.put(TagNames.MANUFAC,convertManufact(null,REQUIRED));
	...
	public TagData convertID(String value,boolean prop) {
		return new TagData(TagNames.OME_ELEM_GENERICEXCITATIONSOURCE,TagNames.ID,value,prop,TagData.TEXTFIELD);
	}

	public TagData convertManufact(String value, boolean prop)
	{
		return new TagData(TagNames.OME_ELEM_GENERICEXCITATIONSOURCE,TagNames.MANUFAC,value,prop,TagData.TEXTFIELD);
	}
	...
	public TagData convertPower(Power value, boolean prop)
	{
		...
		TagData t = new TagData(TagNames.OME_ELEM_GENERICEXCITATIONSOURCE,TagNames.POWER,val,ome.model.units.Power.class,prop,TagData.TEXTFIELD);
	...
	{
		return new TagData(TagNames.OME_ELEM_GENERICEXCITATIONSOURCE,TagNames.MODEL,value,prop,TagData.TEXTFIELD);
	}

	//TODO
	public TagData convertMap(List<MapPair> value, boolean prop)
	{
		String val="";
		return new TagData(TagNames.OME_ELEM_GENERICEXCITATIONSOURCE,TagNames.MAP,val,prop,TagData.TEXTFIELD);
	}
	...
	public TagData convertWavelengthSett(Length value, boolean prop)
	{
		...
		TagData t = new TagData(TagNames.OME_ELEM_GENERICEXCITATIONSOURCE,TagNames.SET_WAVELENGTH,val,ome.model.units.Length.class,prop,TagData.TEXTFIELD);
	...
	public TagData convertAttenuation(PercentFraction value, boolean prop)
	{
		...
		TagData t = new TagData(TagNames.OME_ELEM_GENERICEXCITATIONSOURCE,TagNames.ATTENUATION,val,prop,TagData.TEXTFIELD);
