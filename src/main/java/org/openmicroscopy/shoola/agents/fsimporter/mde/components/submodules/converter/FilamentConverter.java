...
/**
 * Convert OME-XML Filament object to list of TagData. 
 * @author Susanne Kunis<susannekunis at gmail dot com>
 *
 */
public class FilamentConverter extends DataConverter{
	...
	public LinkedHashMap<String, TagData> convertData(Filament l,LightSourceSettings settings)
	{
		...
		if(l!=null) {
			...
			try{ tagMap.put(TagNames.F_TYPE,convertType(l.getType(), REQUIRED));
			} catch (NullPointerException e) { 
				tagMap.put(TagNames.F_TYPE,convertType(null, REQUIRED));
			}	
		}else {
			tagMap.put(TagNames.ID,convertID(null,REQUIRED));
			tagMap.put(TagNames.MANUFAC,convertManufact(null,REQUIRED));
	...
	public TagData convertID(String value,boolean prop) {
		return new TagData(TagNames.OME_ELEM_FILAMENT,TagNames.ID,value,prop,TagData.TEXTFIELD);
	}

	public TagData convertManufact(String value, boolean prop)
	{
		return new TagData(TagNames.OME_ELEM_FILAMENT,TagNames.MANUFAC,value,prop,TagData.TEXTFIELD);
	}
	public TagData convertModel(String value, boolean prop)
	{
		return new TagData(TagNames.OME_ELEM_FILAMENT,TagNames.MODEL,value,prop,TagData.TEXTFIELD);
	}
	
	public TagData convertType(Enumeration value, boolean prop)
	{
		String val= (value != null)? ((FilamentType) value).getValue() : "";
		return new TagData(TagNames.OME_ELEM_FILAMENT,TagNames.F_TYPE,val,prop,TagData.COMBOBOX,OMEValueConverter.getNames(FilamentType.class));
	}
...
//		Unit unit=(value!=null) ? value.unit() :TagNames.POWER_UNIT;
		TagData t = new TagData(TagNames.OME_ELEM_FILAMENT,TagNames.POWER,val,ome.model.units.Power.class,prop,TagData.TEXTFIELD);
...
//		Unit unit=(value!=null) ? value.unit():TagNames.WAVELENGTH_UNIT;
		TagData t = new TagData(TagNames.OME_ELEM_FILAMENT,TagNames.SET_WAVELENGTH,val,ome.model.units.Length.class,prop,TagData.TEXTFIELD);
	...
	public TagData convertAttenuation(PercentFraction value, boolean prop)
	{
		...
		TagData t = new TagData(TagNames.OME_ELEM_FILAMENT,TagNames.ATTENUATION,val,prop,TagData.TEXTFIELD);
